...
func _ready():
	...
	load_dialog()
	$TextBubble/bg/AnimationPlayer.play("anim")
	$Options/bg/AnimationPlayer.play("anim")
...
func update_name(event):
	...
	if event.has('name'):
		var name = parse_text(event['name'])
		$TextBubble/NameLabel.bbcode_text = name
#		$AnimatedSprite.visible = true
#		$TextBubble/AnimatedSprite.anmiation = name
		$TextBubble/AnimatedSprite.play(name)
...
#		$AnimatedSprite.visible = false
		$TextBubble/AnimatedSprite.play() 
	...
	match event:
		...
		{'question', ..}:
			...
			for o in event['options']:
				...
				if event.has('variable'):
					...
					if o['value'] == '0':
						button.connect("pressed", self, "exitDialog")
					else:
						# Continue
		...
		{'action'}:
			match event['action']:
				...
				'go_to_Cards':
					Global.mode = 'move'
					Music.music_position = Music.get_playback_position()
					Music.stop_music()
					Global.goto_scene("res://Scenes/CardsGame.tscn")
					queue_free()	
				'go_to_Towers':
					Global.mode = 'move'
					Music.music_position = Music.get_playback_position()
					Music.stop_music()
					Global.goto_scene("res://Scenes/Towers.tscn")
					queue_free()	
				'go_to_Fachada':
					Global.mode = 'move'
					Music.music_position = Music.get_playback_position()
					Music.stop_music()
					Global.goto_scene("res://Scenes/Fachada.tscn")
					queue_free()
				'go_to_Bancada':
					Global.mode = 'move'
					Music.music_position = Music.get_playback_position()
					Music.stop_music()
					Global.goto_scene("res://Scenes/Bancada.tscn")
					queue_free()
				'next':
				...
				'end_game_fachada':
					...
					queue_free()
				'end_talk':
					get_parent().get_parent().end_talk()
					queue_free()
