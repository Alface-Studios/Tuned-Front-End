...
onready var secondsLabel = get_node("seconds")
var sprite_dict = {"Pickable1": "bricks", "Pickable2": "collumn", "Pickable3": "corner", "Pickable4": "statue", "Pickable5": "vao", "Pickable6": "window"}
var docks = []
var current_obj
...
func _ready():
	...
	for i in sprite_dict:
		...
		a.set_position(Vector2(100,150*count))
#		if anim == "collumn1":
...
func setDialog(obj):
	...
	dialog.extenal_file = "res://addons/dialogs/fachada_" + obj + ".json"
...
func endGame():
	Global.mode = 'move';
	if "fachada_time" in Global.custom_variables:		
		if Global.custom_variables.fachada_time > seconds:
			Global.custom_variables['fachada_time'] = seconds
	else:
		Global.custom_variables['fachada_time'] = seconds
